...
const NotificationButton = ({ onClick, unread }) => {
  const [position, setPosition] = useState({ bottom: 20, left: 100 });
  ...
  return (
    ...
    >
      {unread > 0 && (
        <div className="absolute top-0 right-0 translate-x-1/4 -translate-y-1/4 size-4 rounded-full bg-primary text-xs text-on-primary">
          {unread}
        </div>
      )}
