...
const OrderTag = ({ order, loading}) => {
  ...
  return (
    <div className=" flex flex-col bg-on-surface/60 text-surface shadow-md rounded-lg p-2 gap-2 h-fit">
      <div className="flex flex-wrap justify-between items-center gap-4">
        <h3>OrderID: {order._id}</h3>
        <h3 className="opacity-70 text-sm">{formattedDate(order.createdAt)}</h3>
      </div>
      <Divider />
